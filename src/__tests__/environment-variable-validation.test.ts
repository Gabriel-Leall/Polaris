...
const validGeminiApiKeyGenerator = fc.oneof(
  fc.string({ minLength: 25, maxLength: 100 }).map(s => s.replace(/[^A-Za-z0-9_-]/g, 'A')),
  fc.constantFrom(
...
describe('Environment Variable Validation', () => {
  ...
  test('Property 14: Environment variable validation - Supabase client creation validates inputs', () => {
    fc.assert(
      fc.property(
        ...
        fc.oneof(validSupabaseAnonKeyGenerator, invalidSupabaseAnonKeyGenerator),
        (url: string, key: string) => {
          ...
          try {
            ...
            if (!shouldSucceed) {
              console.log(`Expected validation to fail for URL: ${url.substring(0, 30)}..., Key: ${key.substring(0, 30)}...`)
              return false
            }
            ...
            if (shouldSucceed) {
              console.log(`Unexpected validation failure for valid inputs: ${error}`)
              return false
            }
  ...
  test('Property 14: Environment variable validation - missing required variables are detected', () => {
    fc.assert(
      fc.property(
        fc.record({
          NEXT_PUBLIC_SUPABASE_URL: fc.option(validSupabaseUrlGenerator, { nil: undefined }),
          NEXT_PUBLIC_SUPABASE_ANON_KEY: fc.option(validSupabaseAnonKeyGenerator, { nil: undefined }),
          GEMINI_API_KEY: fc.option(validGeminiApiKeyGenerator, { nil: undefined })
        }),
        (env: Record<string, string | undefined>) => {
          const validation = validateEnvironmentConfig(env)
          
          const hasRequiredVars = Boolean(env.NEXT_PUBLIC_SUPABASE_URL && env.NEXT_PUBLIC_SUPABASE_ANON_KEY)
          const shouldBeValid = hasRequiredVars && 
