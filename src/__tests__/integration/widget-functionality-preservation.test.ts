...
function preservesFunctionalityPatterns(content: string): boolean {
  ...
  const hasEventHandlers = /\bon[A-Z]\w*\s*=/.test(cleanContent) || /handle\w+/.test(cleanContent) || /dispatch|set\w+/.test(cleanContent)
  const hasEffects = /\buseEffect\b/.test(cleanContent)
  const hasProperExports = /export\s+default/.test(cleanContent) || /export\s+const|export\s+function|export\s*{/.test(cleanContent)
