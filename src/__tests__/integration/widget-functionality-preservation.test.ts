...
function hasProperStateManagement(content: string): boolean {
  ...
  const hasProperEventHandlers =
    /const\s+handle\w+\s*=/.test(cleanContent) ||
    /function\s+handle\w+/.test(cleanContent);
...
function hasProperInterfaces(content: string): boolean {
  ...
  const hasPropsInterface =
    ...
    /({[^}]+})\s*:\s*{\s*[^}]+}/.test(content); // Destructured props with inline types

  // Check for some form of typing
...
function preservesFunctionalityPatterns(content: string): boolean {
  ...
  const hasEventHandlers =
    /\bon[A-Z]\w*\s*=/.test(cleanContent) ||
    /handle\w+/.test(cleanContent) ||
    /dispatch|set\w+/.test(cleanContent);
  const hasEffects = /\buseEffect\b/.test(cleanContent);
  const hasProperExports =
    /export\s+default/.test(cleanContent) ||
    /export\s+const|export\s+function|export\s*{/.test(cleanContent);

  // Widget should have interactive functionality
  return (hasEventHandlers || hasEffects) && hasProperExports;
}
...
function usesProperStyling(content: string): boolean {
  ...
  const hasTailwindClasses =
    /className\s*=\s*{?\s*cn\(/.test(content) ||
    /className\s*=\s*["'`][^"'`]*bg-|text-|border-/.test(content);

  // Check for semantic color tokens
  // Check for semantic color tokens - inclusive of standard shadcn tokens
  const hasSemanticTokens =
    /(bg|text|border|ring|outline)-/.test(content) &&
    /(card|popover|primary|secondary|muted|accent|destructive|background|foreground|border|input|ring|radius)/.test(
      content,
    );
...
function hasTimerFunctionality(content: string, filename: string): boolean {
  ...
  const hasTimerState =
    /useState.*[Tt]imer|useState.*[Tt]ime|useState.*[Ss]econds|useState.*[Mm]inutes|TimerState/.test(
      cleanContent,
    );
  const hasInterval = /setInterval|clearInterval|useEffect/.test(cleanContent);
  const hasPlayPause =
    /[Pp]lay|[Pp]ause|[Ss]tart|[Ss]top|isRunning|running/.test(cleanContent);
  const hasTimeFormatting = /padStart|toString|format|formatTime/.test(
    cleanContent,
  );
...
describe("Widget Functionality Preservation", () => {
  test("Property 8: All widgets should have proper state management", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...widgetFiles), (filePath: string) => {
        ...
        if (filename === "ZenTimerWidget.tsx") {
          ...
          if (!hasProperState) {
            console.log(
              `Widget ${filePath} does not have proper state management`,
            );
          }
    ...
    fc.assert(
      fc.property(fc.constantFrom(...widgetFiles), (filePath: string) => {
        ...
        if (filename === "ZenTimerWidget.tsx") {
          ...
          if (!hasInterfaces) {
            console.log(
              `Widget ${filePath} does not have proper TypeScript interfaces`,
            );
          }
      ...
      fc.property(fc.constantFrom(...widgetFiles), (filePath: string) => {
        ...
        if (filename === "ZenTimerWidget.tsx") {
          ...
          if (!preservesPatterns) {
            console.log(
              `Widget ${filePath} does not preserve functionality patterns`,
            );
          }
  ...
  test("Property 8: All widgets should use proper styling with semantic tokens", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...widgetFiles), (filePath: string) => {
        ...
        if (!usesStyling) {
          console.log(
            `Widget ${filePath} does not use proper styling with semantic tokens`,
          );
        }
  ...
  test("Property 8: Timer widgets should have timer-specific functionality", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...widgetFiles), (filePath: string) => {
        ...
        // Only test timer widgets, excluding sub-components
        if (
          filename.toLowerCase().includes("timer") &&
          ...
          if (!hasTimerFunc) {
            console.log(
              `Timer widget ${filePath} does not have proper timer functionality`,
            );
      ...
      fc.property(
        ...
        (minutes: number, seconds: number) => {
          ...
          const hasProperPadding =
            formattedMinutes.length === 2 && formattedSeconds.length === 2;

          if (!isValidFormat || !hasProperPadding) {
            console.log(
              `Invalid time format: ${formatted} for ${minutes}:${seconds}`,
            );
          }
