...
describe("Widget Baseline Functionality", () => {
  ...
  test("Widget naming conventions should be consistent", () => {
    ...
    widgets.forEach((widgetName) => {
      ...
      expect(widgetName.endsWith("Widget") || widgetName.endsWith("Dock")).toBe(
        true,
      );
    });
  });
  ...
  test("Widget components should follow React component patterns", () => {
    ...
    widgets.forEach((widgetName) => {
      ...
      if (componentContent) {
        ...
        if (!hasReactPatterns) {
          console.log(`Widget ${widgetName} missing React patterns`);
        }
        expect(
          hasReactPatterns,
          `Widget ${widgetName} missing React patterns`,
        ).toBe(true);
        ...
        if (!hasComponentDefinition) {
          console.log(`Widget ${widgetName} missing component definition`);
        }
        expect(
          hasComponentDefinition,
          `Widget ${widgetName} missing component definition`,
        ).toBe(true);
      }
    });
  });
