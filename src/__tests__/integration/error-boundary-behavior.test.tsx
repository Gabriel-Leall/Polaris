...
describe("ErrorBoundary", () => {
  ...
  it("retries when retry button is clicked", async () => {
    ...
    // Depending on React scheduling, the boundary may recover immediately.
    // Assert that we eventually reach the recovered state.
    await waitFor(() => {
      expect(screen.getByText("Retry successful")).toBeInTheDocument();
    });

    // If the retry button is present, clicking it should keep the UI stable.
