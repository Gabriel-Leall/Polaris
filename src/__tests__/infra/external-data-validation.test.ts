...
import { createJobApplication } from "@/app/actions/jobApplications";
import { createUserPreferences } from "@/app/actions/userPreferences";
import {
  createTaskSchema,
...
describe("External Data Validation", () => {
  ...
  test("Property 12: Task creation validates all required fields", async () => {
    ...
    for (const invalidInput of invalidInputs) {
      await expect(() => createTask(invalidInput)).rejects.toBeInstanceOf(
        Error
      );
    }
  });
  ...
  test("Property 12: Job application creation validates all fields", async () => {
    ...
    for (const invalidInput of invalidInputs) {
      await expect(() =>
        createJobApplication(invalidInput)
  ...
  test("Property 12: User preferences creation validates all fields", async () => {
    ...
    for (const invalidInput of invalidInputs) {
      await expect(() =>
        createUserPreferences(invalidInput)
      ).rejects.toBeInstanceOf(Error);
    }
  });
  ...
  test("Property 12: Update operations validate partial data", async () => {
    ...
    for (const invalidUpdate of invalidUpdates) {
      await expect(() =>
        updateTask(invalidUpdate.id, invalidUpdate)
      ).rejects.toBeInstanceOf(Error);
    }
  });
  ...
  test("Property 12: Schemas reject malformed data consistently", () => {
    ...
    for (const malformedInput of malformedInputs) {
      if ("label" in malformedInput) {
        ...
        expect(
          createJobApplicationSchema.safeParse(malformedInput).success
        ).toBe(false);
      } else if ("theme" in malformedInput) {
        expect(
          createUserPreferencesSchema.safeParse(malformedInput).success
        ).toBe(false);
      }
    }
  });
